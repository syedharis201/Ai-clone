...
function addMessage(type, text) {
    ...
    if (type === 'ai' && text !== '🔊 Speaking...') {
        ...
        const animate = setInterval(() => {
            if (i < text.length) {
                textP.textContent = text.substring(0, i + 1);
                textP.appendChild(timeSpan);
                i++;
...
function removeInterimMessage() {
    if (state.interimMsgEl && state.interimMsgEl.parentNode) {
        state.interimMsgEl.parentNode.removeChild(state.interimMsgEl);
    }
    state.interimMsgEl = null;
    state.interimTextP = null;
}

function updateInterimMessage(text) {
    if (!state.interimMsgEl) {
        if (dom.transcriptEmpty) dom.transcriptEmpty.style.display = 'none';

        const msgEl = document.createElement('div');
        msgEl.className = `message user interim`;
        ...
        const roleSpan = document.createElement('span');
        roleSpan.className = 'message-role';
        roleSpan.textContent = 'You (speaking...)';

        const textP = document.createElement('p');
        textP.className = 'interim-text';
        textP.textContent = text;
        ...
        dom.transcriptBody.appendChild(msgEl);
        state.interimMsgEl = msgEl;
        state.interimTextP = textP;
    } else {
        state.interimTextP.textContent = text;
    }
...
async function sendToN8N(audioBlob, mimeType = 'audio/webm') {
    ...
    while (retries <= CONFIG.MAX_RETRIES) {
        ...
        try {
            ...
            console.log('[App] Raw response (first 500 chars):', rawText.substring(0, 500));

            // Robust JSON parsing — handles up to 3 levels of string wrapping
            // n8n's respondWith:"json" + JSON.stringify($json) often double-encodes
            let data;
            try {
                data = JSON.parse(rawText);
                // Keep unwrapping if it's still a string (double/triple encoding)
                let unwrapCount = 0;
                while (typeof data === 'string' && unwrapCount < 3) {
                    data = JSON.parse(data);
                    unwrapCount++;
                    console.log(`[App] Unwrapped JSON string (level ${unwrapCount})`);
                }
            } catch (parseErr) {
                console.error('[App] JSON parse error:', parseErr);
                console.error('[App] Raw text was:', rawText.substring(0, 200));
            ...
            // Flatten the response — handle various n8n output formats
            ...
            if (responseData.json && typeof responseData.json === 'object') responseData = responseData.json;

            // Debug: log ALL keys in the response
            console.log('[App] Response keys:', Object.keys(responseData));
            console.log('[App] Full parsed responseData:', JSON.stringify(responseData, null, 2));

            // Deep search helper — finds a value by key name anywhere in the object tree
            const deepSearch = (obj, targetKeys, depth = 0) => {
                ...
                for (const key of Object.keys(obj)) {
                    ...
                    const found = deepSearch(obj[key], targetKeys, depth + 1);
            ...
            if (!audioUrl) {
                audioUrl = deepSearch(data, ['audiourl', 'audio_url']);
                if (audioUrl) console.log('[App] Found audioUrl via deep search');
            }

            // Extract userText — try direct keys first, then deep search
            let userText = responseData.userText || responseData.user_text || responseData.transcription || '';
            if (!userText) {
                userText = deepSearch(data, ['usertext', 'user_text', 'transcription', 'transcript']) || '';
                if (userText) console.log('[App] Found userText via deep search');
            }

            // Extract aiText — try ALL possible keys from n8n LLM chain output
            let aiText = responseData.aiText
            ...
            // If still empty, deep search the entire response tree
            if (!aiText) {
                aiText = deepSearch(data, [
                    'aitext', 'ai_text', 'output', 'response', 'text',
                    'message', 'reply', 'content', 'answer'
                ]) || '';
                if (aiText) console.log('[App] Found aiText via deep search:', aiText.substring(0, 50));
            }

            // If output is an object (like n8n LLM chain sometimes returns), stringify it
            if (typeof aiText === 'object') {
                aiText = aiText.text || aiText.output || aiText.content || JSON.stringify(aiText);
            }

            // ── FALLBACK: reconstruct aiText from visemes.characters ──
            // n8n's LLM chain v1.9 uses a different output key, so aiText is often empty
            // But the spoken text is always in visemes.characters as individual characters
            if (!aiText && responseData.visemes) {
                const visemes = responseData.visemes;
                if (visemes.characters && Array.isArray(visemes.characters)) {
                    aiText = visemes.characters.join('');
                    console.log('[App] ✓ Reconstructed aiText from visemes.characters:', aiText);
                }
            }

            console.log('[App] ── EXTRACTED DATA ──');
            console.log('[App] audioUrl:', audioUrl ? audioUrl.substring(0, 80) : 'MISSING');
            console.log('[App] userText:', userText ? `"${userText}"` : 'EMPTY');
            console.log('[App] aiText:', aiText ? `"${aiText}"` : 'EMPTY');

            // Guard: don't play the same audio URL twice in a row
            if (audioUrl && audioUrl === state.lastAudioUrl) {
                console.warn('[App] Same audioUrl as last time — cache buster will be added');
            }

            // ── ALWAYS show messages in chat ──
            removeInterimMessage();
            ...
            const showAiText = () => {
                if (aiText) {
                    ...
                    addMessage('ai', '🔊 Speaking...');
                    console.warn('[App] ⚠ No aiText found in response! Check n8n Prepare Response node.');
                }
            };
            ...
            if (avatar && audioUrl) {
                ...
                try {
                    await avatar.playAudioSync(audioUrl, showAiText);
                } catch (playErr) {
